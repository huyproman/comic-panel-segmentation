"""Manuscript-faithful U-Net++ architecture for Comic panel extraction (M1 Operational Model).

Normative manuscript specifications:
- Encoder feature widths: 32, 64, 128, 256, 512
- Two 3x3 Conv2D per convolutional block
- Batch Normalization after each Conv2D
- ReLU activation after Batch Normalization
- Nested and dense skip connections
- No deep supervision (exactly 1 output tensor)
- 1x1 Conv2D with Sigmoid activation at output
- Zero Dropout layers
- No embedded Lambda(/255) normalization
"""
...
def conv_block(x: keras.KerasTensor, filters: int, name: str) -> keras.KerasTensor:
    """Standard manuscript convolutional block:
...
def build_manuscript_unetpp(
    input_shape: Tuple[int, int, int] = (448, 448, 3),
    nb_filter=(32, 64, 128, 256, 512)
) -> keras.Model:
    """Build manuscript-faithful U-Net++ model.
    ...
    # Level 4 Bottleneck (512 filters)
